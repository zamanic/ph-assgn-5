...
const YourStack = ({ selectedStack, setSelectedStack, setSelectedIds }: YourStackProps) => {
  ...
  const handleRemoveAll = () => {
    ...
    setSelectedStack([]);
    setSelectedIds([]);
    toast.success("All technologies cleared from your stack", {
      position: "top-center",
      autoClose: 3000,
      transition: Bounce,
    });
  ...
  return (
    <aside className="sticky top-24 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm self-start">
      <h3 className="text-2xl font-extrabold text-slate-900 mb-1">Your Stack</h3>
      {selectedStack.length === 0 ? (
        <p className="text-base text-slate-400 mb-6">No technologies selected yet.</p>
      ) : (
        <p className="text-base text-slate-400 mb-6">
          <span className="font-semibold brand-gradient-text">{selectedStack.length}</span>{" "}
      ...
      {selectedStack.length === 0 ? (
        <div className="border-2 border-dashed border-slate-200 rounded-2xl py-16 px-4 flex items-center justify-center mb-6">
          <p className="text-lg text-slate-400 font-medium">Your stack is empty.</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <h4 className="text-lg font-bold text-slate-900 truncate">{tech.name}</h4>
                  <p className="text-sm text-slate-400 truncate">{tech.category}</p>
                </div>
              </div>
              <button
